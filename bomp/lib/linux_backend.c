...
struct backend * backendptr = malloc(sizeof(sizeof(struct backend)));
    if (!backendptr) {
	printf("%s: TLS allocation error\n", __func__);
	exit (0);
    }
    memset(backendptr, 0, sizeof(struct backend));
...
printf("SELF %p FS id %u addr 0x%lx ERRNO %p\n", THREAD_SELF, TLS_GET_FS(), selector, __errno_location ());

int i;
for (i = 0 ; i < sizeof( tcbhead_t); i++)
