...
ngx_epoll_aio_init(ngx_cycle_t *cycle, ngx_epoll_conf_t *epcf)
{
    ...
    //printf("%s epoll_ctl %d op %d\n", __func__, ngx_eventfd, EPOLL_CTL_ADD);
    if (epoll_ctl(ep, EPOLL_CTL_ADD, ngx_eventfd, &ee) != -1) {
        return;
    }
    //printf("%s epoll_ctl error %d\n", __func__, ngx_eventfd);
...
ngx_epoll_notify_init(ngx_log_t *log)
{
    ...
    //printf("%s epoll_ctl %d op %d\n", __func__, notify_fd, EPOLL_CTL_ADD);
...
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    ngx_log_debug(NGX_LOG_DEBUG_CORE, cycle->log, 0, "%d epoll_wait %ld\n", events, syscall(__NR_gettid));
