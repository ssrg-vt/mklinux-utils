...
static ngx_thread_mutex_t       ngx_thread_pool_done_mtx;
...
ngx_thread_pool_init(ngx_thread_pool_t *tp, ngx_log_t *log, ngx_pool_t *pool)
{
    ...
    if (ngx_thread_mutex_create(&ngx_thread_pool_done_mtx, log) != NGX_OK) {
        return NGX_ERROR;
    }
...
ngx_thread_pool_cycle(void *data)
{
    ...
    for ( ;; ) {
        ...
        if (ngx_thread_mutex_lock(&ngx_thread_pool_done_mtx, tp->log) != NGX_OK) {
            return NULL;
        }
        ...
        *ngx_thread_pool_done.last = task;
        ...
        if (ngx_thread_mutex_unlock(&ngx_thread_pool_done_mtx, tp->log) != NGX_OK) {
            return NULL;
        }
...
ngx_thread_pool_handler(ngx_event_t *ev)
{
    ...
    if (ngx_thread_mutex_lock(&ngx_thread_pool_done_mtx, ev->log) != NGX_OK) {
        return;
    }
    ...
    if (ngx_thread_mutex_unlock(&ngx_thread_pool_done_mtx, ev->log) != NGX_OK) {
        return;
    }
