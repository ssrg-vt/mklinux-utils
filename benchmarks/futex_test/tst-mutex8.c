...
static int
check_type (const char *mas, pthread_mutexattr_t *ma,int cpu)
{
...
2nd mutex_destroy of condvar-used mutex did not return EBUSY for %s\n",
 ...
 printf("before cancel m={%p} b={%p} c={%c} \n",m,&b,&c);
...
//  printf("m={%p} b={%p} c={%c} \n",m,&b,&c);
 ...
 int res = check_type ("normal", NULL, cpu );
  ...
  res |= check_type ("recursive", &ma, cpu);
  ...
  res |= check_type ("error-checking", &ma, cpu);
  if (pthread_mutexattr_destroy (&ma) != 0)
    {
