...
// Lifted kernel
///////////////////////////////////////////////////////////////////////////////
void main_kernel(int dim, int* h_nzcnt, int* h_ptr, int* h_indices, float* h_data, float* h_x_vector, float* h_Ax_vector, int* h_perm)
{
	int i;
...
//  generate_vector(h_x_vector, dim);
  ...
  int p;
	...
	for(p=0;p<50;p++)
	{
		main_kernel(dim, h_nzcnt, h_ptr, h_indices, h_data, h_x_vector, h_Ax_vector, h_perm);
/*#pragma omp parallel for
		for (i = 0; i < dim; i++) {
      int k;
		  float sum = 0.0f;
